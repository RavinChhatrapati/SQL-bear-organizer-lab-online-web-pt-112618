...
def selects_oldest_bear_and_returns_name_and_age
 "SELECT bears.name, bears.age FROM bears WHERE age = (SELECT MAX(age) FROM bears) AND name IS NOT NULL"
end

def select_youngest_bear_and_returns_name_and_age
  "SELECT bears.name, bears.age FROM bears WHERE age = (SELECT MIN(age) FROM bears) AND name IS NOT NULL"
end
